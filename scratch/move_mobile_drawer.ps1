# PowerShell script to move mobile menu drawer outside of the sticky header container to prevent rendering/transparency bugs
...
Get-ChildItem -Filter *.html | ForEach-Object {
    ...
    if ($content -match $headerPattern) {
        $content = [System.Text.RegularExpressions.Regex]::Replace($content, $headerPattern, $headerOnly)
        [System.IO.File]::WriteAllText($filePath, $content)
        Write-Host "Moved mobile drawer outside sticky header in: $name"
    }
}
